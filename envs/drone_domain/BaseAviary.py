...
from envs.drone_domain.drone_utils.enums import DroneModel, Physics, ImageType

'''基于gymnasium的多旋翼仿真环境基类，封装pybullet连接，URDF模型加载，物理步进，相机与录制，状态缓存等功能；
支持多机、多种物理模型与GUI和无头两种运行模型；
作为RL环境骨架，动作、观测空间、奖励终止条件由子类实现
'''
class BaseAviary(gym.Env):
    ...
    def __init__(self,
                 ...
                 pyb_freq: int = 240,  # 物理仿真频率
                 ctrl_freq: int = 240,  # 控制更新频率
                 gui=False,
                 record=False,
                 obstacles=False,
                 user_debug_gui=True,
        ...
        #### Constants #############################################
        self.G = 9.8
        self.RAD2DEG = 180/np.pi  # 弧度转角度
        self.DEG2RAD = np.pi/180  # 角度转弧度
        self.CTRL_FREQ = ctrl_freq
        self.PYB_FREQ = pyb_freq
        if self.PYB_FREQ % self.CTRL_FREQ != 0:
            raise ValueError('[ERROR] in BaseAviary._init__(), pyb_freq is not divisible by env_freq.')
        ...
        if self.GUI:
            ...
            p.resetDebugVisualizerCamera(cameraDistance=3,  # 设置摄像机位置和视角
                                         cameraYaw=-30,
                                         cameraPitch=-30,
    ...
    def reset(self, seed:int=None, options:dict=None):
        ...
        #### Housekeeping ##########################################
        self._housekeeping()  # 初始化和重置无人机仿真环境
        #### Update and store the drones kinematic information #####
        self._updateAndStoreKinematicInformation()
        #### Start video recording #################################
        self._startVideoRecording()
    ...
    def step(self, action):
        ...
        for _ in range(self.PYB_STEPS_PER_CTRL):
            ...
            if self.PHYSICS != Physics.DYN:
                p.stepSimulation(physicsClientId=self.CLIENT)  # 运行模拟
    ...
    def _housekeeping(self):
        ...
        urdf_file_path = os.path.join(assets_dir, self.URDF)
        self.DRONE_IDS = np.array([p.loadURDF(urdf_file_path,
                        self.INIT_XYZS[i, :],
    ...
    def _parseURDFParameters(self):
        ...
        """
        # envs.multidrone not install by pip install -e . ,change to use the path find
        current_file_dir = os.path.dirname(os.path.abspath(__file__))
        assets_dir = os.path.join(current_file_dir, 'assets')
        urdf_file_path = os.path.join(assets_dir, self.URDF)
        URDF_TREE = etxml.parse(urdf_file_path).getroot()
        # URDF_TREE = etxml.parse(pkg_resources.resource_filename('multidrone', 'assets/' + self.URDF)).getroot()
