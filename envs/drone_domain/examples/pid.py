...
from envs.drone_domain.CtrlAviary import CtrlAviary
from envs.drone_domain.control.DSLPIDControl import DSLPIDControl
from envs.drone_domain.drone_utils.Logger import Logger
from envs.drone_domain.drone_utils.utils import sync, str2bool
from envs.drone_domain.BaseFeature import *
...
def run(
    ...
    PERIOD = 10 # 圆轨迹的周期为 10 秒
    NUM_WP = control_freq_hz*PERIOD # 一圈轨迹总共包含的路径点（Waypoint）数量，和控制频率成正比
    TARGET_POS = np.zeros((NUM_WP,3)) # 初始化目标位置数组，大小为 (NUM_WP, 3)，每个路径点的三维坐标 (x, y, z)。
    for i in range(NUM_WP): # 圆形轨迹生成 (i/NUM_WP)*(2*np.pi)：将 i 映射到 [0, 2π] 之间，实现一圈圆 +np.pi/2：使起点在圆的顶端（相当于将圆顺时针旋转了90度）+INIT_XYZS[0, 0] 和 +INIT_XYZS[0, 1]：把圆心平移到给定的初始位置
        TARGET_POS[i, :] = R*np.cos((i/NUM_WP)*(2*np.pi)+np.pi/2)+INIT_XYZS[0, 0], R*np.sin((i/NUM_WP)*(2*np.pi)+np.pi/2)-R+INIT_XYZS[0, 1], 0 # -R：将圆下移一个半径，使轨迹从初始 y 坐标起始。
    wp_counters = np.array([int((i*NUM_WP/6)%NUM_WP) for i in range(num_drones)]) # 初始化每个无人机的起始路径点索引 每个无人机的索引相差 NUM_WP/6（即六等分圆轨迹），确保它们在圆上均匀分布、间隔一致地开始飞行。
