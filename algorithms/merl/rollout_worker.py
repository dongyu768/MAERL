...
def rollout_worker(args, id, type, task_pipe, result_pipe, data_bucket, models_bucket, store_transitions, random_baseline):
	...
	elif type == 'evo': NUM_EVALS = 10 if not args['env_choice'] == 'motivate' else 1 
	...
	while True:
		...
		if type == 'test' or type == 'pg': 
			team = [models_bucket[0] for _ in range(args['num_agents'])]
		elif type == "evo": 
			team = [models_bucket[0][teams_blueprint[0]] for _ in range(args['num_agents'])]
		else: 
		...
		while True: #unless done
			...
			if random_baseline:
				...
				joint_action = [team[i][0].noisy_action(joint_state[i,:], head=i).detach().numpy() for i in range(args['num_agents'])]
			else:
				joint_action = [team[i].clean_action(joint_state[i, :], head=i).detach().numpy() for i in range(args['num_agents'])]
